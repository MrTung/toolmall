...
/**
 *  手动同步去通过网络获取数据，并同步的回调，如果调用了这个函数，就不需要注册回调
 *
 *  @param successBlock
 *  @param failedBlock  
 */
+ (void)fetchDynamicReleasePackageSyncWithRequest:(DRRequest)requestBlock
                                        onSuccess:(DRFetchSuccessBlock)successBlock
                                       onFailed:(DRFailedBlock)failedBlock
                                        onFinally:(DRFinallyBlock)finallyBlock;


/**
 *  convenient method to fetch pakage
 */
