...
//@property WebViewJavascriptBridgeBase *bridgeBase;
...
- (void)drawTitle
{
    if (!_topTitle)
    {
        ...
        _topTitle.center = CGPointMake(CGRectGetWidth(self.view.frame)/2, CGRectGetHeight(self.view.frame)/2.0 + 150);
        
        //3.5inch iphone
        if ([UIScreen mainScreen].bounds.size.height <= 568 )
        {
            _topTitle.center = CGPointMake(CGRectGetWidth(self.view.frame)/2, CGRectGetHeight(self.view.frame)/2.0 + 100);
...
- (void)showNextVCWithScanResult:(LBXScanResult*)strResult
{
    ...
    NSString * _strScan = strResult.strScanned;
    ...
    if ([self.type isEqualToString:@"webView"]) {
        ...
        [data setValue:_strScan forKey:@"content"];

        NSLog(@"data - %@", data);
        
        [self.delegate sendValue:data];
        
//        [CommonUtils saveDataWithKeyPath:@"onSaoMa" valueDictionary:data];
        self.returnScanBarCodeValue(_strScan);
        [self.navigationController popViewControllerAnimated:YES];
    ...
    if ([_strScan hasPrefix:website_url]){
        NSString *newprodurl = [website_url stringByAppendingString:@"productinfo/"];
        NSString *produrl = [website_url stringByAppendingString:@"product/content/"];
        NSString *wapprodurl = [website_url stringByAppendingString:@"wap/product/content/"];
        if ([_strScan hasPrefix:newprodurl]){
        ...
        } else if ([_strScan hasPrefix:produrl] || [_strScan hasPrefix:wapprodurl]){
            NSArray *array = [_strScan componentsSeparatedByString:@"/"];
            NSString *productStr = [array objectAtIndex:array.count - 1];
            productStr = [productStr stringByReplacingOccurrencesOfString:@".jhtm" withString:@""];
            int prodId = [productStr intValue];
            //            ProdInfo *prodInfo = [[ProdInfo alloc] init];
