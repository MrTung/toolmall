...
//    [self.navigationController pushViewController:vc animated:YES];
    ...
    if ([strResult.strScanned hasPrefix:website_url]){
        
        NSString *produrl = [website_url stringByAppendingString:@"product/content/"];
        if ([strResult.strScanned hasPrefix:produrl]){
    ...
    } else if ([strResult.strScanned hasPrefix:@"couponExchangeByCode"]){
        NSArray *array = [strResult.strScanned componentsSeparatedByString:@":"];
    ...
    //    其他二维码
        ...
        _otherUrl = strResult.strScanned;
        UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"可能存在风险，是否继续打开网址" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"打开", nil];
