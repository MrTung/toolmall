...
/** 换一批热门搜索按钮 */
@property (nonatomic, weak) UIButton *refleshButton;
...
- (UIButton *)refleshButton
{
    if (!_refleshButton) {
        // 添加清空按钮
        UIButton *refleshButton = [[UIButton alloc] init];
        refleshButton.titleLabel.font = self.searchHistoryHeader.font;
        [refleshButton setTitleColor:PYTextColor forState:UIControlStateNormal];
        [refleshButton setTitle:@"换一批" forState:UIControlStateNormal];
        [refleshButton setImage:[UIImage imageNamed:@"PYSearch.bundle/empty"] forState:UIControlStateNormal];
        [refleshButton addTarget:self action:@selector(emptySearchHistoryDidClick) forControlEvents:UIControlEventTouchDown];
        [refleshButton sizeToFit];
        refleshButton.py_y = 0;
        refleshButton.py_x = PYScreenW - refleshButton.py_width;
        _refleshButton = refleshButton;
    }
    return _refleshButton;
}
...
- (void)setup
{
    ...
    UIButton *refleshButton = [[UIButton alloc] init];
    refleshButton.titleLabel.font = [UIFont systemFontOfSize:13];
    [refleshButton setTitleColor:PYTextColor forState:UIControlStateNormal];
    [refleshButton setTitle:@"换一批" forState:UIControlStateNormal];
    [refleshButton setImage:[UIImage imageNamed:@"PYSearch.bundle/reflesh"] forState:UIControlStateNormal];
    [refleshButton addTarget:self action:@selector(refleshDidClick) forControlEvents:UIControlEventTouchDown];
    [refleshButton sizeToFit];
    refleshButton.py_y = 0;
    refleshButton.py_x = PYScreenW - refleshButton.py_width - PYMargin - 20;
    [contentView addSubview:refleshButton];
...
/** 点击换一批按钮 */
- (void)refleshDidClick
{
    if (self.refleshClick) {
        self.refleshClick();
    }
    PYSearchLog(@"换一批");
}
