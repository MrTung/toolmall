...
#define errorMsgUrl @"https://192.168.2.160:8080/toolmall/wap/error/getErrorMessage.jhtm"
...
- (void)validateAndSaveCriticalApplicationData:(NSException *)exception {
    ...
    NSString *exceptionInfo = [NSString stringWithFormat:@"\n--------Log Exception---------\nappInfo             :\n%@\n\nexception name      :%@\nexception reason    :%@\nexception userInfo  :%@\ncallStackSymbols    :%@\ncallStackReturnAddresses    :%@\n\n--------End Log Exception-----", getAppInfo(),exception.name, exception.reason, exception.userInfo ? : @"no user info", [exception callStackSymbols], [exception callStackReturnAddresses]];
    
//    NSString *str = [NSString stringWithFormat:@"%@ %@(%@)", [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleDisplayName"], [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleShortVersionString"], [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleShortVersionString"]];
//    
//    NSString *str2 = [NSString stringWithFormat:@"%@ %@", [UIDevice currentDevice].systemName, [UIDevice currentDevice].systemVersion];
    
//    NSDictionary *loaddic = [NSDictionary dictionaryWithObjectsAndKeys: str, @"versionSdk", [UIDevice currentDevice].name, @"deviceId", [UIDevice currentDevice].localizedModel, @"model", str2, @"versionName",exception.name, @"message", exception.reason, @"localizedMessage", exception.userInfo, @"errorMessage", @"", @"CPU_ABI", @"", @"BOARD", @"", @"cellLocation", @"", @"subscriberId", @"", @"versionCode",nil];

//    NSString *object = [self dictionaryToJson:loaddic];
//    
//    int cpu_type_t = [self getCpuType];
//    
//    NSLog(@"%@", exceptionInfo);
...
- (void)upLoagDataPostJson:(id)error {
    
    // 1. 准备网址字符串
    //    NSString *urlString = [NSString stringWithFormat:AddAddressUrl, self.type];
    
    // 2. 创建 NSURL 对象
    NSURL *url = [NSURL URLWithString:errorMsgUrl];
    ...
    // 5. 将参数字符串转换为二进制流对象
    NSString *bodyString = [NSString stringWithFormat:@"jsonObject=%@", error];
