...
@interface IndexViewController ()<UIAlertViewDelegate, NSXMLParserDelegate>
{
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(clickTabBar) name:TabBarDidSelectedNotification object:nil];
    
    placeholderImage = [UIImage imageNamed:@"index_defImage.png"];
    
    self.backScrollView.frame = CGRectMake(0, 0, [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT]-64-49);
...
- (void)clickTabBar {
    ...
    if (self.tabBarController.selectedViewController == self.lastVC && self.tabBarController.selectedIndex == 0) {
...
- (void)configureScrollView {
    ...
    self.imageScroll.contentOffset = CGPointMake(self.pagecontroll.currentPage * [AppStatic SCREEN_WIDTH], 0);
...
- (void)configInfoOfrecommands {
    ...
    CGFloat itemWidth = [AppStatic SCREEN_WIDTH] *210/750;
    CGFloat itemHeight = [AppStatic SCREEN_HEIGHT] *254/1334;
...
- (void)configOfHot1 {
    ...
    if (_arrayHot.count) {
        ...
        hotac1.frame = CGRectMake(0, 0, ([AppStatic SCREEN_WIDTH])/2.0 - 0.5, [AppStatic SCREEN_WIDTH] / 2.0 - 0.5);
    ...
    if (_arrayHot.count>2) {
        ...
        for (int i=1; i<3; i++) {
            ...
            if (i==1) {
                ...
                hotac2.frame =CGRectMake(([AppStatic SCREEN_WIDTH])/2.0, 0, ([AppStatic SCREEN_WIDTH])/2.0 - 0.5, [AppStatic SCREEN_WIDTH]/4.0 - 0.5);
                hotac2.titleLabel.textColor = redColorSelf;
            } else {
                hotac2.frame =CGRectMake(([AppStatic SCREEN_WIDTH])/2.0, [AppStatic SCREEN_WIDTH]/4.0, ([AppStatic SCREEN_WIDTH])/2.0 - 0.5, [AppStatic SCREEN_WIDTH]/4.0 - 0.5);
    ...
    if (_arrayHot.count>5) {
        ...
        for (int i=3; i<6; i++) {
            ...
            if (i==3) {
                ...
                hotac3.frame =CGRectMake(0, ([AppStatic SCREEN_WIDTH])/2.0, ([AppStatic SCREEN_WIDTH])/3.0 - 0.5, ([AppStatic SCREEN_WIDTH])/3.0*1.2 - 0.5);
                ...
                hotac3.frame =CGRectMake(([AppStatic SCREEN_WIDTH])/3.0, ([AppStatic SCREEN_WIDTH])/2.0, ([AppStatic SCREEN_WIDTH])/3.0 - 0.5, ([AppStatic SCREEN_WIDTH])/3.0*1.2 - 0.5);
            }
            else {
                ...
                hotac3.frame =CGRectMake(([AppStatic SCREEN_WIDTH])/3.0*2, ([AppStatic SCREEN_WIDTH])/2.0, ([AppStatic SCREEN_WIDTH])/3.0 - 0.5, ([AppStatic SCREEN_WIDTH])/3.0*1.2 - 0.5);
            }
...
- (void)configInfoOfTodayBrand{
    ...
    CGFloat itemW = [AppStatic SCREEN_WIDTH]/2.0;
    CGFloat itemHFloat = [AppStatic SCREEN_WIDTH]/4.0;
...
//    CGFloat itemW = [AppStatic SCREEN_WIDTH]/3.0;
//    CGFloat itemH = [AppStatic SCREEN_WIDTH]/3.0*1.2;
    ...
    for (int i=0; i<_arrayTop.count; i++) {
        ...
        if (i<3) {
            ...
            acView.frame = CGRectMake(([AppStatic SCREEN_WIDTH])/3.0*i, 0, ([AppStatic SCREEN_WIDTH])/3.0, ([AppStatic SCREEN_WIDTH])/3.0*1.2);
        }else if (i>=3 && i<6){
            acView.frame = CGRectMake(([AppStatic SCREEN_WIDTH])/3.0*(i-3), ([AppStatic SCREEN_WIDTH])/3.0*1.2, ([AppStatic SCREEN_WIDTH])/3.0, ([AppStatic SCREEN_WIDTH])/3.0*1.2);
        }else if (i>=6 && i<9){
            acView.frame = CGRectMake(([AppStatic SCREEN_WIDTH])/3.0*(i-6), ([AppStatic SCREEN_WIDTH])/3.0*1.2*2, ([AppStatic SCREEN_WIDTH])/3.0, ([AppStatic SCREEN_WIDTH])/3.0*1.2);
        }
        else {
            acView.frame = CGRectMake(([AppStatic SCREEN_WIDTH])/3.0*(i-9), ([AppStatic SCREEN_WIDTH])/3.0*1.2*3, ([AppStatic SCREEN_WIDTH])/3.0, ([AppStatic SCREEN_WIDTH])/3.0*1.2);
        }
...
//    NSLog(@"offset - %f : offset1 - %f", offset, offset1);
    ...
    if (offset > offset1 + [AppStatic SCREEN_HEIGHT] *60/568) {
        self.backScrollView.contentSize = CGSizeMake(0, tHeight+[AppStatic SCREEN_HEIGHT] *110/568);
    }
    if (offset < offset1 - [AppStatic SCREEN_HEIGHT] *20/568){
...
//    self.navigationItem.hidesBackButton = YES;
    ...
    if ([self.type isEqualToString:@"webView"]) {
        ...
        self.backScrollView.frame = CGRectMake(0, 0, [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT]-64);
    }
}
...
- (void)createUI{
    //    滚动图片位置
    self.imageScroll.frame = CGRectMake(0, 0, [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT] *150/667);
...
//    [self.view addGestureRecognizer: _imageScroll.panGestureRecognizer];
    ...
    self.pagecontroll = [[UIPageControl alloc] initWithFrame:CGRectMake(CGRectGetMaxX(_imageScroll.frame)/2-[AppStatic SCREEN_WIDTH] *50/320, CGRectGetMaxY(_imageScroll.frame)-[AppStatic SCREEN_HEIGHT] *20/568, [AppStatic SCREEN_WIDTH] *100/320, [AppStatic SCREEN_HEIGHT] *20/568)];
...
//    [self.backScrollView addSubview:self.btnClickImage];
    ...
    // 快速进货
    _faststockView.frame = CGRectMake(0, CGRectGetMaxY(_imageScroll.frame), [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT] *90/1334);
//    [self.backScrollView addSubview:_faststockView];

    //    四个按钮的位置(不会变化)
    CGFloat viewWidth = [AppStatic SCREEN_WIDTH]/4;
    CGFloat viewHeight = [AppStatic SCREEN_HEIGHT] *160/1334;
    CGFloat imgPointX = [AppStatic SCREEN_WIDTH]/8 - [AppStatic SCREEN_WIDTH] *40/750;
    CGFloat imgWidth = [AppStatic SCREEN_WIDTH] *80/750;
    CGFloat imgHeight = [AppStatic SCREEN_WIDTH] *80/750;
    _menu.frame = CGRectMake(0, CGRectGetMaxY(_imageScroll.frame), [AppStatic SCREEN_WIDTH], viewHeight);
    _view1.frame = CGRectMake(0, 0, viewWidth, viewHeight);
    _imgMenu1.frame = CGRectMake(imgPointX, [AppStatic SCREEN_HEIGHT] *10/568, imgWidth, imgHeight);
    _view2.frame = CGRectMake(viewWidth, 0, viewWidth, viewHeight);
    _imgMenu2.frame = CGRectMake(imgPointX, [AppStatic SCREEN_HEIGHT] *10/568, imgWidth, imgHeight);
    
    _view3.frame = CGRectMake(viewWidth*2, 0, viewWidth, viewHeight);
    _imgMenu3.frame = CGRectMake(imgPointX, [AppStatic SCREEN_HEIGHT] *10/568, imgWidth, imgHeight);
    
    _view4.frame = CGRectMake(viewWidth*3, 0, viewWidth, viewHeight);
    _imgMenu4.frame = CGRectMake(imgPointX, [AppStatic SCREEN_HEIGHT] *10/568, imgWidth, imgHeight);
    [self.backScrollView addSubview:_menu];
    
    CGFloat imgW = [AppStatic SCREEN_HEIGHT] *46/1334;
    CGFloat imgH = [AppStatic SCREEN_HEIGHT] *46/1334;
    CGFloat imgSize = [AppStatic SCREEN_WIDTH] *16/320;
    //    土猫推荐UI 计算
    self.toolmallRecommendView.frame = CGRectMake(0, CGRectGetMaxY(self.menu.frame)+[AppStatic SCREEN_HEIGHT] *20/1334, [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT] *90/1334);
    self.tmRimg1.frame = CGRectMake([AppStatic SCREEN_WIDTH] *10/320, ([AppStatic SCREEN_HEIGHT] *90/1334 - imgH)/2, imgW, imgH);
    
    self.tmRtitleLabel.frame = CGRectMake(CGRectGetMaxX(_tmRimg1.frame)+[AppStatic SCREEN_WIDTH] *5/320, 0, [AppStatic SCREEN_WIDTH] *150/320, [AppStatic SCREEN_HEIGHT] *90/1334);
    self.tmRimg2.frame = CGRectMake([AppStatic SCREEN_WIDTH]-imgSize-[AppStatic SCREEN_WIDTH] *10/320, ([AppStatic SCREEN_HEIGHT] *90/1334 - imgSize)/2, imgSize, imgSize);
    self.tmRmoreLabel.frame = CGRectMake(CGRectGetMinX(_tmRimg2.frame)-[AppStatic SCREEN_WIDTH] *100/320, 0, [AppStatic SCREEN_WIDTH] *100/320, [AppStatic SCREEN_HEIGHT] *90/1334);
    [self.backScrollView addSubview:_toolmallRecommendView];
    
    _scrollFoot = [[UIScrollView alloc] init];
    _scrollFoot.frame = CGRectMake(0, CGRectGetMaxY(_toolmallRecommendView.frame)+0.5, [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT] * 254/1334);
    ...
    _advImage1.frame = CGRectMake(0, CGRectGetMaxY(_scrollFoot.frame), [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT] *120/1334);
    ...
    // 重新给热门活动frame
    self.hotActivityView.frame = CGRectMake(0, CGRectGetMaxY(_advImage1.frame)+[AppStatic SCREEN_HEIGHT] *20/1334, [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT] *90/1334);
    self.tmHimg1.frame = CGRectMake([AppStatic SCREEN_WIDTH] *10/320, ([AppStatic SCREEN_HEIGHT] *90/1334 - imgH)/2, imgW, imgH);
    
    self.tmHtitleLabel.frame = CGRectMake(CGRectGetMaxX(_tmRimg1.frame)+[AppStatic SCREEN_WIDTH] *5/320, 0, [AppStatic SCREEN_WIDTH] *150/320, [AppStatic SCREEN_HEIGHT] *90/1334);
    self.tmHimg2.frame = CGRectMake([AppStatic SCREEN_WIDTH]-imgSize-[AppStatic SCREEN_WIDTH] *10/320, ([AppStatic SCREEN_HEIGHT] *90/1334 - imgSize)/2, imgSize, imgSize);
    self.tmHmoreLabel.frame = CGRectMake(CGRectGetMinX(_tmRimg2.frame)-[AppStatic SCREEN_WIDTH] *100/320, 0, [AppStatic SCREEN_WIDTH] *100/320, [AppStatic SCREEN_HEIGHT] *90/1334);
    [self.backScrollView addSubview:_hotActivityView];
    
    _imageListView = [[UIView alloc] init];
    _imageListView.frame =CGRectMake(0, CGRectGetMaxY(self.hotActivityView.frame)+0.5, [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT] *675/1334);
//    _imageListView.backgroundColor = [UIColor lightGrayColor];
    ...
    _advImage2.frame = CGRectMake(0, CGRectGetMaxY(_imageListView.frame), [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT] *120/1334);
...
//    [self.backScrollView addSubview:_advImage2];
    ...
    //    今日大牌UI 计算
    self.todaybrandView.frame = CGRectMake(0, CGRectGetMaxY(_imageListView.frame)+[AppStatic SCREEN_HEIGHT] *20/1334, [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT] *90/1334);
    self.tmTimg1.frame = CGRectMake([AppStatic SCREEN_WIDTH] *10/320, ([AppStatic SCREEN_HEIGHT] *90/1334 - imgH)/2, imgW, imgH);
    self.tmTtitleLabel.frame = CGRectMake(CGRectGetMaxX(_tmRimg1.frame)+[AppStatic SCREEN_WIDTH] *5/320, 0, [AppStatic SCREEN_WIDTH] *150/320, [AppStatic SCREEN_HEIGHT] *90/1334);
    self.tmTimg2.frame = CGRectMake([AppStatic SCREEN_WIDTH]-imgSize-[AppStatic SCREEN_WIDTH] *10/320, ([AppStatic SCREEN_HEIGHT] *90/1334 - imgSize)/2, imgSize, imgSize);
    self.tmTmoreLabel.frame = CGRectMake(CGRectGetMinX(_tmRimg2.frame)-[AppStatic SCREEN_WIDTH] *100/320, 0, [AppStatic SCREEN_WIDTH] *100/320, [AppStatic SCREEN_HEIGHT] *90/1334);
    [self.backScrollView addSubview:_todaybrandView];
    
    self.todayListView = [[UIView alloc] init];
    _todayListView.frame =CGRectMake(0, CGRectGetMaxY(self.todaybrandView.frame)+0.5, [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_WIDTH]/2);
//    _todayListView.backgroundColor = [UIColor lightGrayColor];
    [self.backScrollView addSubview:_todayListView];
    
    //    全部分类UI 计算
    self.allclassifyView.frame = CGRectMake(0, CGRectGetMaxY(self.todayListView.frame)+[AppStatic SCREEN_HEIGHT] *20/1334, [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT] *90/1334);
    self.tmAimg1.frame = CGRectMake([AppStatic SCREEN_WIDTH] *10/320, ([AppStatic SCREEN_HEIGHT] *90/1334 - imgH)/2, imgW, imgH);
    self.tmAtitleLabel.frame = CGRectMake(CGRectGetMaxX(_tmRimg1.frame)+[AppStatic SCREEN_WIDTH] *5/320, 0, [AppStatic SCREEN_WIDTH] *150/320, [AppStatic SCREEN_HEIGHT] *90/1334);
    self.tmAimg2.frame = CGRectMake([AppStatic SCREEN_WIDTH]-imgSize-[AppStatic SCREEN_WIDTH] *10/320, ([AppStatic SCREEN_HEIGHT] *90/1334 - imgSize)/2, imgSize, imgSize);
    self.tmAmoreLabel.frame = CGRectMake(CGRectGetMinX(_tmRimg2.frame)-[AppStatic SCREEN_WIDTH] *100/320, 0, [AppStatic SCREEN_WIDTH] *100/320, [AppStatic SCREEN_HEIGHT] *90/1334);
    self.allClassifyListView = [[UIView alloc] init];
    
    _allClassifyListView.frame = CGRectMake(0, CGRectGetMaxY(self.allclassifyView.frame)+0.5, [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_WIDTH]/3*1.2*4-0.5);
...
//    NSLog(@"%f", tHeight);
    ...
    // moreViewUI计算
    self.moreView.frame = CGRectMake(0, CGRectGetMaxY(self.allClassifyListView.frame)+[AppStatic SCREEN_HEIGHT] *5/568, [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT] *100/568);
...
//    self.tmLtitleLabel.font
    ...
    //    在线客服UI 计算
    self.onlineserviceView.frame = CGRectMake(0, [AppStatic SCREEN_HEIGHT] *10/568, [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT] *40/568);
    self.tmLimg1.frame = CGRectMake([AppStatic SCREEN_WIDTH] *70/320, ([AppStatic SCREEN_HEIGHT] *40/568 - [AppStatic SCREEN_WIDTH] *18/320)/2, [AppStatic SCREEN_WIDTH] *18/320, [AppStatic SCREEN_WIDTH] *18/320);
    self.tmLtitleLabel.frame = CGRectMake(CGRectGetMaxX(_tmLimg1.frame)+[AppStatic SCREEN_WIDTH] *5/320, 0, labSize.width, [AppStatic SCREEN_HEIGHT] *40/568);
    self.onlineserviceBtn.frame = CGRectMake(CGRectGetMaxX(_tmLtitleLabel.frame)+[AppStatic SCREEN_WIDTH] *3/320, 0, [AppStatic SCREEN_WIDTH] *120/320, [AppStatic SCREEN_HEIGHT] *40/568);
    
    //    热线电话UI 计算
    self.hottelephoneView.frame = CGRectMake(0, [AppStatic SCREEN_HEIGHT] *50/568, [AppStatic SCREEN_WIDTH], [AppStatic SCREEN_HEIGHT] *40/568);
    self.tmPimg1.frame = CGRectMake([AppStatic SCREEN_WIDTH] *70/320, ([AppStatic SCREEN_HEIGHT] *40/568 - [AppStatic SCREEN_WIDTH] *18/320)/2, [AppStatic SCREEN_WIDTH] *18/320, [AppStatic SCREEN_WIDTH] *18/320);
    self.tmPtitleLabel.frame = CGRectMake(CGRectGetMaxX(_tmLimg1.frame)+[AppStatic SCREEN_WIDTH] *5/320, 0, labSize.width, [AppStatic SCREEN_HEIGHT] *40/568);
    self.hottelephoneBtn.frame = CGRectMake(CGRectGetMaxX(_tmLtitleLabel.frame)+[AppStatic SCREEN_WIDTH] *3/320, 0, [AppStatic SCREEN_WIDTH] *120/320, [AppStatic SCREEN_HEIGHT] *40/568);
...
//    btnWebLogo.frame = CGRectMake(0, 0, [AppStatic SCREEN_WIDTH] *50/320, [AppStatic SCREEN_HEIGHT] *24/568);
...
//    btnWebLogo.adjustsImageWhenHighlighted = NO;
    ...
    logoImg.frame = CGRectMake(0, 0, [AppStatic SCREEN_WIDTH] *50/320, [AppStatic SCREEN_HEIGHT] *25/568);
...
//    _searchBtn.userInteractionEnabled = NO;
    CGFloat marginWidth;
    if ([AppStatic SCREEN_WIDTH] == 414) {
        marginWidth = 52;
    } else {
        marginWidth = 44;
    }
    _searchBtn.frame = CGRectMake(0, 0, [AppStatic SCREEN_WIDTH] - [AppStatic SCREEN_WIDTH] *71/320 - marginWidth,[AppStatic SCREEN_HEIGHT] *28/568);
//    _searchBtn.backgroundColor = [UIColor yellowColor];
    ...
    _searchBtn.contentEdgeInsets = UIEdgeInsetsMake(0, [AppStatic SCREEN_WIDTH] *30/320, 0, 0);
...
//    btnMSG.backgroundColor = [UIColor cyanColor];
    btnMSG.frame = CGRectMake(0, 0, [AppStatic SCREEN_WIDTH] *21/320, [AppStatic SCREEN_HEIGHT] *28/568);
...
//    if (session.uid < 0){
//        ShopLogin *shopLogin = [[ShopLogin alloc] initWithNibName:@"ShopLogin" bundle:nil];
//        shopLogin.nextPageWhenLogined = @"msg";
//        [self.navigationController pushViewController:shopLogin animated:YES];
//        return;
//    }
...
- (void)setTextValue {
    ...
    //
    NSString *index_hotactive_more = [[TextDataBase shareTextDataBase] searchTextStrByModelPath:@"index_hotactive_more"];
    // 今日大牌
...
////截取字符串
//- (NSString * )makePartInString:(NSString *)string start:(NSString*)start end:(NSString *)end{
//    string = [string componentsSeparatedByString:start][1];
//    string = [string componentsSeparatedByString:end][0];
//    return string;
//}
